...
cat > src/train_residual_v4.py <<'PY'
...
echo
echo "=========================================="
echo "LiqHeat ML V4 — Residual Exception Model"
echo "=========================================="

python src/train_residual_v4.py \
  2>&1 | tee logs/train_residual_v4.log
...
report = json.loads(
    Path(
        "reports/residual_v4_report.json"
    ).read_text()
)

baseline = report["direction_results"][
    "nearest_pool_baseline"
]
hybrid = report["direction_results"][
    "residual_hybrid"
]
...
print(
    ...
    "MCC=",
    round(baseline["mcc"], 4),
)

print(
    "Residual hybrid:",
    "accuracy=",
    round(hybrid["accuracy"], 4),
    "balanced=",
    round(
        hybrid["balanced_accuracy"],
        4,
    ),
    "MCC=",
    round(hybrid["mcc"], 4),
